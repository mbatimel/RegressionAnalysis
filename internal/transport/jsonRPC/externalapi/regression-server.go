...
func (srv *serverRegression) MlrRegression(ctx context.Context, observer string, vars []string, dataPoints []models.DataPoint) (string, error) {
	return srv.mlrRegression(ctx, observer, vars, dataPoints)
}

func (srv *serverRegression) RidgeRegression(ctx context.Context, xData [][]float64, yData [][]float64, alpha float64, tol float64, normalize bool) (*mat.Dense, error) {
	return srv.ridgeRegression(ctx, XData, YData, alpha, tol, normalize)
}

func (srv *serverRegression) LassoRegression(ctx context.Context, xData [][]float64, yData [][]float64, alpha float64, tol float64, normalize bool) (*mat.Dense, error) {
	return srv.lassoRegression(ctx, XData, YData, alpha, tol, normalize)
}

func (srv *serverRegression) ElasticNetRegression(ctx context.Context, params models.ElasticNetParams) (map[string][]float64, error) {
