...
func (rs *regressionService) OnlyMlrRegression(ctx context.Context, observer string, vars []string, dataPoints []models.DataPoint) (map[string]interface{}, error) {
	r := new(linearmodel.Regression)
	r.SetObserved(observer)
	for i, v := range vars {
		r.SetVar(i, v)
	...
	rs.logger.Info().Msg("Starting checking on classifier MLR")
	MLR, err := MLRChecking(Xtrain, Ytrain, Xtest, Ytest, testPoints)
	if err != nil {
		return nil, fmt.Errorf("MLR checking is dead")
	}
	res := map[string]interface{}{
		"data":        r,
		"names":       r.GetNames(),
		"coeff":       r.GetCoeffs(),
		"MLR":         MLR,
		// "graphics":   makeGraphicsForLinear(dataPoints,r.GetCoeffs()),
		"datapoints": r.GetDataPoints(),
	}

	return res, nil
}
func (rs *regressionService) OnlyMlrRegressionCSV(ctx context.Context, file []byte) (map[string]interface{}, error) {
	r := new(linearmodel.Regression)
	buf := bufio.NewReaderSize(bytes.NewReader(file), 16*1024*1024) // 16 MB буфер
	reader := csv.NewReader(buf)
...
func (rs *regressionService) OnlyMlrRegressionExcel(ctx context.Context, file []byte) (map[string]interface{}, error) {
	r := new(linearmodel.Regression)
	dataPoints := make([]models.DataPoint, 0)
	reader := bytes.NewReader(file)
	xlFile, err := excelize.OpenReader(reader)
